...
const Navbar = () => {
  ...
  const ToggleNotifcation = useNotificationsStore(
    (state) => state.ToggleNotifcation,
  );
  const notification = useNotificationsStore((state) => state.notification);
  const searchAppointments = useAppointmentsStore(
    (state) => state.searchAppointments,
  );
        ...
        <div className="fixed inset-0 z-2000 flex items-center justify-center p-4" data-aos="fade">
          ...
          <div className="relative bg-white w-full max-w-sm rounded-3xl p-8 shadow-2xl text-center" data-aos="zoom-in" data-aos-delay="60">
            ...
            <div className="flex gap-4">
              <button
                onClick={() => logout()}
                className="flex-1 px-6 py-3 rounded-2xl border-2 cursor-pointer border-slate-100 font-semibold"
              >
                Yes
              </button>
              <button
                onClick={() => setIsLogoutModalOpen(false)}
                className="flex-1 px-6 py-3 rounded-2xl cursor-pointer bg-red-500 text-white font-semibold"
              >
                Cancel
              </button>
            </div>
